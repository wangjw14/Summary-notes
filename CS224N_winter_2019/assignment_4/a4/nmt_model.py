...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = output[0].permute((1,0,2))
        init_decoder_hidden = self.h_projection(torch.cat((hn[0],hn[1]),-1))
        init_decoder_cell = self.c_projection(torch.cat((cn[0],cn[1]),-1))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.einsum("bh,bsh->bs",dec_hidden,enc_hiddens_proj)
        ...
        a_t = torch.einsum("bs,bsh->bh",alpha_t,enc_hiddens)
